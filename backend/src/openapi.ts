export const openApiSpec = {
    ...
    schemas: {
      GameSummary: {
        ...
        properties: {
          ...
          mode:               { type: 'string', enum: ['pvp', 'vs_computer', 'multiplayer'] },
          computerLevel:      { type: 'integer', nullable: true, minimum: 1, maximum: 10 },
          moveCount:          { type: 'integer' },
          createdAt:          { type: 'string', format: 'date-time' },
          playerColor:        { type: 'string', enum: ['w', 'b'], nullable: true },
          waitingForOpponent: { type: 'boolean' },
        },
      },
      GameState: {
        type: 'object',
  ...
  paths: {
    ...
    '/games/join/{inviteCode}': {
      parameters: [
        { name: 'inviteCode', in: 'path', required: true, schema: { type: 'string' } },
      ],
      post: {
        summary: 'Join a multiplayer game',
        description: 'Join an existing multiplayer game as the second player using an invite code.',
        responses: {
          '200': {
            description: 'Game state with playerColor set to "b"',
            content: {
              'application/json': { schema: { $ref: '#/components/schemas/GameState' } },
            },
          },
          '400': { description: 'Cannot join your own game' },
          '401': { description: 'Missing or invalid JWT' },
          '404': { description: 'Invalid invite code' },
          '409': { description: 'Game is already full' },
        },
      },
    },
    '/games/{gameId}': {
