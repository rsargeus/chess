...
esbuild.context({
  ...
  http.createServer((req, res) => {
    ...
    if (req.url.startsWith('/games') || req.url.startsWith('/checkout') || req.url.startsWith('/me') || req.url.startsWith('/webhooks') || req.url.startsWith('/health') || req.url.startsWith('/analyze')) {
      const options = {
        hostname: 'localhost',
        port: 3000,
        path: req.url,
